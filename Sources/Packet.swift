...
func getMessage(from bytes: ArraySlice<UInt8>) throws -> AtemMessage {
	let title = UInt32(from: bytes[messageTitlePosition])
	if let messageType = messageTypes[title] {
		return messageType.init(with: bytes)
	}
	throw PacketParseError.unknownMessageTitle(try! bytes[messageTitlePosition].toString())
}

struct AtemPacket: CustomDebugStringConvertible {
	var isRepeated: Bool
	let isConnect: Bool
	var number: UInt16?
	var acknowledgement: UInt16?
	let connectionUID: UID
	let messages: [AtemMessage]
	
	init(bytes: [UInt8]) {
		let packetLength = Int(bytes[0] & 0b111) << 8 + Int(bytes[1])
		let type = AtemPacketTypes(rawValue: bytes[0])
...
struct SerialAtemPacket {
	...
	mutating func makeRetransmission() {
		bytes[0] = bytes[0] | AtemPacketTypes.retransmission.rawValue
	}
	
	/// Creates a connect packet to send to an ATEM Switcher
	static func connectToCore(uid: UID, type: AtemPacketTypes) -> [UInt8] {
	...
	static func connectToController(uid: UID, type: AtemPacketTypes) -> [UInt8] {
